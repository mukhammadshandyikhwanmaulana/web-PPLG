...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4"
     x-data="{
         previewOpen: false,
         previewPhoto: '',
         previewTitle: 'Preview Foto Fasilitas',
         openPreview(photo, title) {
             if (!photo) return;
             this.previewPhoto = photo;
             this.previewTitle = title || 'Preview Foto Fasilitas';
    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl shadow-xs border border-slate-300">
        <form id="search-form" method="GET" action="{{ route('admin.fasilitas.index') }}" class="relative w-full">
    ...
    <div class="block md:hidden space-y-3">
        @forelse ($facilities as $item)
            @php 
                $mediaPhoto = $item->photoMedia ?? $item->photo;
            ...
            <div class="bg-white rounded-2xl border border-slate-300 p-4 shadow-xs flex flex-col gap-3">
                <div class="flex items-start gap-3">
                    @if ($photoUrl)
                        <div class="relative group cursor-pointer shrink-0 rounded-xl overflow-hidden border border-slate-200"
                             @click="openPreview(@js($photoUrl), @js($item->name))">
                            <img src="{{ $photoUrl }}" alt="{{ $item->name }}" class="w-16 h-16 object-cover bg-slate-50">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <span class="shrink-0 text-[11px] font-medium text-slate-500 pt-0.5">
                                #{{ $item->sort_order ?? '-' }}
                            </span>
                        </div>
                        @if($item->description)
                ...
                <div class="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
                    ...
                    <form action="{{ route('admin.fasilitas.destroy', $item) }}" method="POST" class="inline-flex items-center m-0 p-0"
...
<script>
    ...
    function handleAutoSearch() {
        ...
        searchTimer = setTimeout(() => {
            sessionStorage.setItem('fasilitas_search_focus', 'true');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (searchInput && sessionStorage.getItem('fasilitas_search_focus') === 'true') {
            searchInput.focus();
            const textLen = searchInput.value.length;
            searchInput.setSelectionRange(textLen, textLen);
            sessionStorage.removeItem('fasilitas_search_focus');
        }
    });
</script>
@endsection