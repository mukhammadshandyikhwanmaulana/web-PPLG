...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Edit Akun Guru</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perbarui informasi data akun dan profil guru ini.</p>
        </div>
    </div>
         ...
         x-data="{
            ...
            onFileSelected(event) {
                ...
                if (file) {
                    ...
                    // Panggil Event Cropper Modal Global (Rasio 1:1 untuk Foto Profil Guru) via CustomEvent
                    window.dispatchEvent(new CustomEvent('open-cropper', {
                        detail: {
                            title: 'Potong Foto Profil Guru Pengganti (1:1)',
                            aspectRatio: 1,
                            file: file,
        ...
        <form action="{{ route('admin.guru.update', $guru) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $guru->name) }}" required class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition @error('name') border-rose-300 bg-rose-50/30 @enderror">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $guru->email) }}" required class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition @error('email') border-rose-300 bg-rose-50/30 @enderror">
                    @error('email')
                        <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div x-data="{ showPassword: false }">
                    <label for="password" class="h-6 flex items-baseline text-sm font-semibold text-slate-900 mb-1.5">Password Baru <span class="text-slate-400 text-xs font-normal ml-1">(opsional)</span></label>
                    <div class="relative">
                    ...
                    <p class="mt-1 text-[11px] text-slate-400">Jika diubah, minimal 8 karakter (kombinasi huruf & angka).</p>
                    @error('password')
                        <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div x-data="{ showConfirmPassword: false }">
                    <label for="password_confirmation" class="h-6 flex items-baseline text-sm font-semibold text-slate-900 mb-1.5">Konfirmasi Password Baru</label>
                    <div class="relative">
            ...
            <div class="flex items-center gap-2 pt-1 select-none">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $guru->is_active) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 cursor-pointer">
            ...
            @php
                $currentPositions = $selectedPositions ?? (array) ($guru->staffMember?->position ?? []);
                $isKajurHasThis = in_array('Ketua Kompetensi Keahlian PPLG', $currentPositions) || in_array('Kepala Kompetensi Keahlian PPLG', $currentPositions) || in_array('Kepala Jurusan PPLG', $currentPositions);
                $isKajurDisabled = ($isKajurExist ?? false) && !$isKajurHasThis;
            @endphp
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="flex flex-wrap gap-3 p-3 bg-slate-50/70 border border-slate-300 rounded-xl">
                        <label class="inline-flex items-center gap-2 {{ $isKajurDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer' }} select-none">
                            <input type="checkbox" name="positions[]" value="Ketua Kompetensi Keahlian PPLG" 
                                   {{ (in_array('Ketua Kompetensi Keahlian PPLG', old('positions', $currentPositions)) || in_array('Kepala Kompetensi Keahlian PPLG', old('positions', $currentPositions)) || in_array('Kepala Jurusan PPLG', old('positions', $currentPositions))) ? 'checked' : '' }}
                                   {{ $isKajurDisabled ? 'disabled' : '' }}
                                   class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 {{ $isKajurDisabled ? 'cursor-not-allowed' : 'cursor-pointer' }}">
                            <span class="text-sm font-medium text-slate-700">
                                Ketua Kompetensi Keahlian
                                @if($isKajurDisabled)
                                    <span class="text-xs text-rose-500 font-normal ml-1">(Terisi oleh guru lain)</span>
                                @endif
                            </span>
                        </label>
                        <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                            <input type="checkbox" name="positions[]" value="Guru Produktif PPLG" 
                                   {{ in_array('Guru Produktif PPLG', old('positions', $currentPositions)) ? 'checked' : '' }}
                        ...
                        <label class="inline-flex items-center gap-2 cursor-pointer select-none">
                            <input type="checkbox" name="positions[]" value="Staff PPLG" 
                                   {{ in_array('Staff PPLG', old('positions', $currentPositions)) ? 'checked' : '' }}
                                   class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 cursor-pointer">
                ...
                <div>
                    ...
                    <input type="text" name="expertise" id="expertise" value="{{ old('expertise', $guru->staffMember?->expertise) }}" class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition @error('expertise') border-rose-300 bg-rose-50/30 @enderror">
                    @error('expertise')
                        <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                @php
                    $photoObj = $guru->staffMember?->photoMedia ?? $guru->staffMember?->photo;
                    $photoPath = $photoObj?->file_path ?? $photoObj?->path;
                    $diskName = $photoObj?->disk ?? 'public';
                    $currentPhotoUrl = $photoPath ? \Illuminate\Support\Facades\Storage::disk($diskName)->url(ltrim($photoPath, '/')) : null;
                @endphp

                @if($currentPhotoUrl)
                    <div x-show="!hasNewImage" class="mb-3 p-3 border border-slate-200 rounded-xl bg-slate-50 inline-flex items-center gap-3">
                        <img src="{{ $currentPhotoUrl }}" alt="{{ $guru->name }}" class="w-16 h-16 object-cover rounded-xl border border-slate-200 shadow-xs">
                        <div>
                            <p class="text-xs font-semibold text-slate-700">Foto Saat Ini</p>
                            <p class="text-[11px] text-slate-400">File foto aktif di profil publik</p>
                        </div>
                    </div>
                @endif

                <div x-show="hasNewImage" x-cloak class="mb-3 flex items-center gap-3">
                    <img x-ref="photoPreviewImg" src="#" alt="Preview Baru" class="w-20 h-20 object-cover rounded-xl border border-slate-300 shadow-xs">
            ...
            {{-- Staff Is Active --}}
            <div class="flex items-center gap-2 pt-1 select-none">
                <input type="checkbox" name="staff_is_active" id="staff_is_active" value="1" {{ old('staff_is_active', $guru->staffMember?->is_active ?? true) ? 'checked' : '' }} class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 cursor-pointer">
