...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Isi formulir di bawah ini untuk menambahkan karya siswa baru.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-300 shadow-xs p-4 sm:p-6">
        <form method="POST" action="{{ route('admin.karya-siswa.store') }}" enctype="multipart/form-data" class="space-y-5"
            ...
            @include('admin.karya-siswa._form', ['studentWork' => null, 'supervisors' => $supervisors ?? []])
