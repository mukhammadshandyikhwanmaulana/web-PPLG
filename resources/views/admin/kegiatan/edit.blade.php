...
<div class="max-w-3xl mx-auto py-4 sm:py-6 px-3 sm:px-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
        <form method="POST" action="{{ route('admin.kegiatan.update', $kegiatan ?? $activity) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', ($kegiatan ?? $activity)->title) }}" required
                ...
                <div>
                    ...
                    <input type="date" name="event_date" id="event_date" value="{{ old('event_date', optional(($kegiatan ?? $activity)->event_date)->format('Y-m-d') ?? ($kegiatan ?? $activity)->event_date) }}" required
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="5" required
                          class="w-full text-sm border border-gray-300 rounded-lg px-3.5 py-2.5 shadow-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition resize-none @error('content') border-red-500 @enderror">{{ old('content', ($kegiatan ?? $activity)->content) }}</textarea>
            ...
            <div>
                ...
                @if (($kegiatan ?? $activity)->photo ?? false)
                    <div class="mb-3">
                        <span class="block text-xs font-medium text-gray-500 mb-1">Foto Saat Ini:</span>
                        <img src="{{ Storage::url(($kegiatan ?? $activity)->photo->file_path) }}" alt="Kegiatan" class="w-32 h-24 object-cover rounded-lg border border-gray-200 shadow-sm">
                    </div>
                @endif
            ...
            <div>
                ...
                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', ($kegiatan ?? $activity)->sort_order) }}" min="0"
