        ...
        <form method="GET" class="flex flex-col md:flex-row items-stretch md:items-center gap-3">
            ...
            <div class="flex items-center gap-2 w-full md:w-auto">
                ...
                @if(request('search') || request('status'))
                    <a href="{{ route('admin.kegiatan.index') }}" class="inline-flex items-center justify-center gap-1 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 text-sm font-medium px-4 py-2.5 rounded-xl transition duration-150">
        ...
        @forelse ($activities as $item)
            <div class="bg-white rounded-2xl border border-slate-300 p-4 shadow-xs flex flex-col gap-3">
                <div class="flex items-start gap-3">
                    @if ($item->cover)
                        <img src="{{ Storage::url($item->cover->file_path) }}" alt="{{ $item->title }}" class="w-16 h-16 object-cover rounded-xl border border-slate-200 shrink-0">
                    @else
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between gap-2 mb-1">
                            ...
                            @endif
                            <span class="text-[11px] text-slate-400 font-medium">Galeri: {{ $item->galleries->count() }}/8</span>
                        </div>
            ...
            <tbody class="divide-y divide-slate-100 text-sm">
                @forelse ($activities as $item)
                    <tr class="hover:bg-slate-50/50 transition">
                        <td class="py-3 px-4">
                            @if ($item->cover)
                                <img src="{{ Storage::url($item->cover->file_path) }}" alt="{{ $item->title }}" class="w-14 h-10 object-cover rounded-xl border border-slate-200 shadow-xs">
                            @else
