...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Tambah Gambar Hero</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Unggah gambar baru untuk latar belakang slider beranda.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-300 shadow-xs p-4 sm:p-6"
        ...
        <form method="POST" action="{{ route('admin.banner.store') }}" enctype="multipart/form-data" class="space-y-5">
            @csrf

            {{-- Upload Single Foto --}}
            <div>
                <label class="block text-sm font-semibold text-slate-900 mb-1.5">Gambar Hero <span class="text-rose-500 ml-1">*</span></label>
                <div class="border-2 border-dashed border-slate-300 hover:border-slate-400 bg-slate-50/50 hover:bg-slate-50 rounded-2xl p-4 sm:p-6 transition-all text-center relative">
                    <input id="image" x-ref="heroInput" name="image" type="file" accept="image/jpeg,image/png,image/webp,image/jpg" required class="hidden" @change="onFileSelected($event)">

                    <div x-show="!hasImage" class="space-y-2 cursor-pointer" @click="$refs.heroInput.click()">
                        ...
                        <p class="text-xs sm:text-sm font-medium text-slate-700"><span class="text-indigo-600 font-semibold underline hover:text-indigo-700">Pilih berkas gambar</span> untuk Hero Section</p>
                    ...
                    <div x-show="hasImage" x-cloak class="space-y-3">
                        <div class="max-w-lg mx-auto">
                            <img x-ref="heroPreviewImg" src="#" alt="Preview Image" class="w-full h-56 object-cover rounded-xl border border-slate-300 shadow-xs">
                        </div>
                        <button type="button" @click="resetImage()" class="text-xs font-medium text-rose-600 hover:underline cursor-pointer">Ganti Gambar</button>
                    </div>
                </div>
