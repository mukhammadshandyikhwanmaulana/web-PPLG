@php
    $sw = $studentWork ?? $sw ?? null;

    // Resolusi Cover Universal
    $coverUrl = null;
    if ($sw) {
        if (isset($sw->cover_url) && $sw->cover_url) {
            $coverUrl = $sw->cover_url;
        } elseif (isset($sw->cover)) {
            $coverObj = is_object($sw->cover) ? ($sw->cover->media ?? $sw->cover) : $sw->cover;
...
<div class="space-y-5">
    ...
    <div>
        <label for="contributor_name" class="block text-sm font-semibold text-slate-900 mb-1.5">
            Nama Kelas / Siswa <span class="text-rose-500 ml-1">*</span>
        </label>
        <input type="text" id="contributor_name" name="contributor_name" value="{{ old('contributor_name', $sw?->contributor_name ?? $sw?->student_name) }}"
               placeholder="Contoh: Ahmad Rizky (XII PPLG 1)"
               class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition @error('contributor_name') border-rose-300 bg-rose-50/30 @enderror" required>
    ...
    <div>
        <label for="description" class="block text-sm font-semibold text-slate-900 mb-1.5">
            Deskripsi Karya <span class="text-rose-500 ml-1">*</span>
        </label>
        <textarea id="description" name="description" rows="4" required
                  placeholder="Jelaskan detail mengenai karya ini..."
                  class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition resize-none @error('description') border-rose-300 bg-rose-50/30 @enderror">{{ old('description', $sw?->description ?? $sw?->content) }}</textarea>
        @error('description')
            <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
        @enderror
    </div>
        ...
        <div class="flex flex-col sm:flex-row items-start gap-4">
            ...
            <div class="flex-1 w-full">
                <label for="cover" class="flex flex-col items-center justify-center px-4 py-4 border-2 border-slate-300 border-dashed rounded-xl bg-slate-50/50 hover:bg-slate-100/80 transition cursor-pointer">
                    ...
                    <span id="cover-file-name" class="text-[11px] text-slate-400 mt-1">Pilih file jika ingin mengisi/mengubah sampul</span>
    ...
    {{-- SECTION 2: GALERI FOTO DOKUMENTASI --}}
    <div class="pt-4 border-t border-slate-200 space-y-3">
        <div>
