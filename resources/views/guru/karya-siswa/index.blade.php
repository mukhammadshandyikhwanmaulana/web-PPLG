...
@section('title', 'Karya Siswa Bimbingan')
...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Karya Siswa Bimbingan</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Kelola daftar produk, aplikasi, dan hasil karya siswa di bawah bimbingan Anda.</p>
        </div>
        <div>
            <a href="{{ route('guru.karya-siswa.create') }}"
    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl shadow-xs border border-slate-300">
        <form id="search-form" method="GET" action="{{ route('guru.karya-siswa.index') }}" class="flex flex-col md:flex-row items-stretch md:items-center gap-3">
            <div class="relative w-full md:flex-1">
                ...
                @if(request('search'))
                    <a href="{{ route('guru.karya-siswa.index', request()->except('search')) }}" 
        ...
        <table class="w-full text-left border-collapse min-w-[900px]">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                    ...
                    <th class="py-3.5 px-4">Siswa / Kontributor</th>
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="py-3 px-4 max-w-xs">
                            ...
                            @if($item->description || $item->content)
                                <div class="text-xs text-slate-500 font-normal line-clamp-1 mt-0.5">
                                    {{ Str::limit(strip_tags($item->description ?? $item->content), 90) }}
                                </div>
                            @endif
                        </td>
                        <td class="py-3 px-4 text-slate-600">
                            {{ $item->contributor_name ?? $item->student_name ?? '-' }}
                        </td>
                        ...
                        <td class="py-3 px-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('guru.karya-siswa.edit', $item) }}"
                                   class="inline-flex items-center text-xs font-semibold text-indigo-600 hover:text-indigo-900 transition leading-none">
                                    Edit
                                </a>
                                <form action="{{ route('guru.karya-siswa.destroy', $item) }}" method="POST" class="inline-flex items-center m-0 p-0"
    ...
    <div x-show="previewOpen" 
        ...
        <div class="relative max-w-4xl w-full bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-slate-800 flex flex-col max-h-[90vh]"
            ...
            <div class="flex items-center justify-between p-4 border-b border-slate-800 text-white shrink-0">
                ...
                <button type="button" @click="previewOpen = false" class="text-slate-400 hover:text-white transition p-1 rounded-lg hover:bg-slate-800 cursor-pointer shrink-0">
            ...
            <div class="relative flex-1 flex items-center justify-center p-4 min-h-[300px] bg-slate-950/50 overflow-hidden">
...
<script>
    ...
    function handleAutoSearch() {
        ...
        searchTimer = setTimeout(() => {
            sessionStorage.setItem('guru_karya_search_focus', 'true');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (searchInput && sessionStorage.getItem('guru_karya_search_focus') === 'true') {
            searchInput.focus();
            const textLen = searchInput.value.length;
            searchInput.setSelectionRange(textLen, textLen);
            sessionStorage.removeItem('guru_karya_search_focus');
        }
    });
</script>
@endsection