...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perbarui informasi karya siswa bimbingan Anda, foto sampul, dan kelola galeri.</p>
        </div>
    </div>

    <!-- Form Card Container -->
    <div class="bg-white rounded-2xl border border-slate-300 shadow-xs p-4 sm:p-6">
        <form method="POST" action="{{ route('guru.karya-siswa.update', $studentWork) }}" enctype="multipart/form-data" class="space-y-5"
              x-data="{
                  removedGalleries: [],
            ...
            @include('guru.karya-siswa._form', ['studentWork' => $studentWork])
