...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Edit Data Prestasi</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perbarui rincian perolehan prestasi bimbingan Anda.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-300 shadow-xs p-4 sm:p-6">
        <form action="{{ route('guru.prestasi.update', $achievement) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="contributor_name" id="contributor_name" value="{{ old('contributor_name', $achievement->contributor_name) }}" required class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition @error('contributor_name') border-rose-300 bg-rose-50/30 @enderror">
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition resize-none @error('description') border-rose-300 bg-rose-50/30 @enderror">{{ old('description', $achievement->description) }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                {{-- Tampilan Dokumen Tersimpan Saat Ini --}}
                @php
                    $mediaDoc = $achievement->document ?? $achievement->media ?? null;
                    $docPath = $mediaDoc?->path ?? $mediaDoc?->file_path ?? null;
                    $diskName = $mediaDoc?->disk ?? 'public';
                    $docUrl = $docPath ? (filter_var($docPath, FILTER_VALIDATE_URL) ? $docPath : \Illuminate\Support\Facades\Storage::disk($diskName)->url($docPath)) : null;
                    $isPdfDoc = ($mediaDoc?->mime_type === 'application/pdf') || (strtolower(pathinfo($docPath ?? '', PATHINFO_EXTENSION)) === 'pdf');
                @endphp

                @if($docUrl)
                    <div x-show="!fileName" class="mb-3 p-3 border border-slate-200 rounded-xl bg-slate-50">
                        ...
                        @else
                            <div>
                                <img src="{{ $docUrl }}" alt="{{ $mediaDoc->alt_text ?? $achievement->title }}" class="w-32 h-24 object-cover rounded-lg border border-slate-200 shadow-xs">
                            </div>
                        @endif
                    </div>
                @endif
