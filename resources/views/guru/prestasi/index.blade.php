...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4" 
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Data Prestasi Siswa & Guru</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Pencatatan perolehan kejuaraan, penghargaan, dan kompetensi bimbingan Anda.</p>
        </div>
        <div>
    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl shadow-xs border border-slate-300">
        <form id="search-form" method="GET" action="{{ route('guru.prestasi.index') }}" class="flex flex-col md:flex-row items-stretch md:items-center gap-3">
            ...
            <div class="relative w-full md:flex-1">
                ...
                @if(request('search'))
                    <a href="{{ route('guru.prestasi.index', request()->except('search')) }}" 
    ...
    <div class="block md:hidden space-y-3">
        @forelse ($achievements as $achievement)
            @php
                ...
                $docUrl = $docPath ? (filter_var($docPath, FILTER_VALIDATE_URL) ? $docPath : \Illuminate\Support\Facades\Storage::disk($diskName)->url($docPath)) : '';
            ...
            <div class="bg-white rounded-2xl border border-slate-300 p-4 shadow-xs flex flex-col gap-3">
                ...
                <div class="text-xs text-slate-600 space-y-1.5 bg-slate-50 p-3 rounded-xl border border-slate-200/60">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-slate-800 font-medium">
                            {{ is_object($achievement->level) && method_exists($achievement->level, 'label') ? $achievement->level->label() : ($achievement->level->value ?? $achievement->level ?? '—') }}
                        </span>
                    </div>
                    <div class="flex justify-between">
                ...
                <div class="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
                    <a href="{{ route('guru.prestasi.edit', $achievement) }}"
                       class="inline-flex items-center text-xs font-semibold text-indigo-600 hover:text-indigo-900 transition leading-none">
                        Edit
                    </a>
                    <form action="{{ route('guru.prestasi.destroy', $achievement) }}" method="POST" class="inline-flex items-center m-0 p-0"
...
<script>
    ...
    function handleAutoSearch() {
        ...
        searchTimer = setTimeout(() => {
            sessionStorage.setItem('guru_prestasi_search_focus', 'true');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (searchInput && sessionStorage.getItem('guru_prestasi_search_focus') === 'true') {
            ...
            sessionStorage.removeItem('guru_prestasi_search_focus');
        }
    });
</script>
@endsection