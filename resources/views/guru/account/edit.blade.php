...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Kelola informasi data diri, foto profil, dan kata sandi akun guru Anda.</p>
        </div>
    </div>
        ...
        <form action="{{ route('guru.account.update') }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="space-y-6">
            ...
            <div class="space-y-6">
                <div class="pb-2 border-b border-slate-200">
                    <h2 class="text-xs font-bold text-slate-400 uppercase tracking-wider">Informasi Akun & Biodata Staf</h2>
                </div>

                {{-- Container Upload & Crop Foto Profil Guru --}}
                <div x-data="{ 
                    photoPreview: @js($user->avatar_url ?? null),
                    removePhoto: false,
                    onAvatarSelected(event) {
                        ...
                        if (file.size > 2 * 1024 * 1024) {
                            alert('Ukuran foto terlalu besar! Maksimal ukuran adalah 2 MB.');
                            event.target.value = '';
                            return;
                        }

                        // Panggil Modal Cropper Global dengan rasio 1:1 (persegi)
                        $dispatch('open-cropper', {
                            title: 'Potong Foto Profil Guru',
                            aspectRatio: 1, // Rasio 1:1 khusus foto profil
                            file: file,
                    ...
                    <div class="relative shrink-0">
                        <template x-if="photoPreview">
                            <img :src="photoPreview" alt="Foto Profil" class="w-20 h-20 rounded-2xl object-cover border-2 border-indigo-500/20 shadow-xs">
                        </template>
                        <template x-if="!photoPreview">
                    ...
                    <div class="space-y-2 flex-1 min-w-0">
                        <label class="block text-sm font-semibold text-slate-900">Foto Profil Guru</label>
                        <p class="text-xs text-slate-500">Unggah foto profil dalam format JPG, PNG, atau WEBP. Maksimal 2 MB.</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label for="name" class="h-6 flex items-baseline text-sm font-semibold text-slate-900 mb-1.5">
                            Nama Lengkap & Gelar <span class="text-rose-500 ml-1">*</span>
                        </label>
                    ...
                    <div>
                        <label for="email" class="h-6 flex items-baseline text-sm font-semibold text-slate-900 mb-1.5">
                            Alamat Email Login <span class="text-rose-500 ml-1">*</span>
                        </label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {{-- Posisi / Jabatan --}}
                    <div>
                        <label for="position" class="h-6 flex items-baseline text-sm font-semibold text-slate-900 mb-1.5">
                            Jabatan / Posisi
                        </label>
                        <input type="text" name="position" id="position" value="{{ old('position', $staff->position ?? '') }}" placeholder="Contoh: Guru Produktif PPLG"
                               class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition bg-white @error('position') border-rose-300 bg-rose-50/30 @enderror">
                        @error('position')
                            <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    {{-- Keahlian --}}
                    <div>
                        <label for="expertise" class="h-6 flex items-baseline text-sm font-semibold text-slate-900 mb-1.5">
                            Bidang Keahlian
                        </label>
                        <input type="text" name="expertise" id="expertise" value="{{ old('expertise', $staff->expertise ?? '') }}" placeholder="Contoh: Laravel, Vue.js, UI/UX"
                               class="w-full text-sm border border-slate-300 rounded-xl px-3.5 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition bg-white @error('expertise') border-rose-300 bg-rose-50/30 @enderror">
                        @error('expertise')
                            <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="space-y-4 pt-4 border-t border-slate-200">
                <div>
                    <h2 class="text-xs font-bold text-slate-400 uppercase tracking-wider">Keamanan & Kata Sandi (Opsional)</h2>
                    <p class="text-xs text-slate-500 mt-1">Isi kolom di bawah ini hanya jika Anda ingin mengubah kata sandi.</p>
                </div>

                {{-- Password Saat Ini --}}
                <div x-data="{ showCurrentPassword: false }">
                    <label for="current_password" class="h-6 flex items-baseline text-sm font-semibold text-slate-900 mb-1.5">
                        Kata Sandi Saat Ini
                    </label>
                    <div class="relative">
                        <input :type="showCurrentPassword ? 'text' : 'password'" 
                               ...
                               class="w-full text-sm border border-slate-300 rounded-xl pl-3.5 pr-10 py-2.5 shadow-xs focus:ring-1 focus:ring-slate-900 focus:border-slate-900 transition bg-white @error('current_password') border-rose-300 bg-rose-50/30 @enderror">
                        ...
                        <button type="button" 
                            ...
                            <svg x-show="showCurrentPassword" x-cloak class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
