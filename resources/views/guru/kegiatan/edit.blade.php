...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Edit Laporan Kegiatan</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Perbarui informasi, status, dan dokumentasi kegiatan bimbingan Anda.</p>
        </div>
    </div>

    <!-- Form Card Container -->
    <div class="bg-white rounded-2xl border border-slate-300 shadow-xs p-4 sm:p-6">
        <form method="POST" action="{{ route('guru.kegiatan.update', $activity) }}" enctype="multipart/form-data" class="space-y-5"
              x-data="{
                  removedGalleries: [],
            ...
            @include('guru.kegiatan._form', ['activity' => $activity])

            <!-- Submit Button Bar -->
            <div class="pt-5 border-t border-slate-200 flex items-center justify-between sm:justify-end gap-3">
                <a href="{{ route('guru.kegiatan.index') }}" 
                   class="inline-flex items-center justify-center px-5 py-2.5 rounded-xl border border-slate-300 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition text-center shrink-0">
                    Batal
                </a>
