...
@section('title', 'Laporan Kegiatan & Berita')
...
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 px-4 sm:px-6 lg:px-8 py-4"
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">Kegiatan & Berita Jurusan</h1>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Dokumentasikan acara, kunjungan, dan agenda pembelajaran PPLG yang Anda ampu.</p>
        </div>
        <div>
            <a href="{{ route('guru.kegiatan.create') }}"
    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl shadow-xs border border-slate-300">
        <form id="search-form" method="GET" action="{{ route('guru.kegiatan.index') }}" class="flex flex-col md:flex-row items-stretch md:items-center gap-3">
            <div class="relative w-full md:flex-1">
                ...
                @if(request('search'))
                    <a href="{{ route('guru.kegiatan.index', request()->except('search')) }}" 
            ...
            <div class="bg-white rounded-2xl border border-slate-300 p-4 shadow-xs flex flex-col gap-3">
                ...
                <div class="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                    ...
                    <div class="flex items-center gap-3 shrink-0">
                        <a href="{{ route('guru.kegiatan.edit', $item) }}"
                           class="inline-flex items-center text-xs font-semibold text-indigo-600 hover:text-indigo-900 transition leading-none">
                            Edit
                        </a>
                        <form action="{{ route('guru.kegiatan.destroy', $item) }}" method="POST" class="inline-flex items-center m-0 p-0"
                ...
                @forelse ($activities as $item)
                    ...
                    <tr class="hover:bg-slate-50/50 transition">
                        ...
                        <td class="py-3 px-4 max-w-xs">
                            ...
                            @if($item->content)
                                <div class="text-xs text-slate-500 font-normal line-clamp-1 mt-0.5" title="{{ strip_tags($item->content) }}">
...
<script>
    ...
    function handleAutoSearch() {
        ...
        searchTimer = setTimeout(() => {
            sessionStorage.setItem('guru_kegiatan_search_focus', 'true');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (searchInput && sessionStorage.getItem('guru_kegiatan_search_focus') === 'true') {
            searchInput.focus();
            const textLen = searchInput.value.length;
            searchInput.setSelectionRange(textLen, textLen);
            sessionStorage.removeItem('guru_kegiatan_search_focus');
        }
    });
</script>
@endsection